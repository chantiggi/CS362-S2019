...
int main(int argc, char** argv)
{
	 ...
	 * effect for the Great Hall card and therefore do not necessarily need
	 * to be randomized */
	...
	int test6Fails;	// function returning 0
	...
	int totTest6Fails = 0;	// function returning 0
	...
	for (int p = 2; p < 5; p++)
	{
		...
		test6Fails = 0;	// function returning 0
		...
		for (int i = 0; i < NUM_RANDOM_TESTS; i++)
		{
			...
			// Test 1: Check # actions - should be +1 based on great_hall card effect
			test1Fails += assertTrue("+1 actions", begState.numActions + 1, endState.numActions);
			...
			// Test 5: Check total # played cards - should be +1 from playing great hall
			test5Fails += assertTrue("+1 total # played cards", begState.playedCardCount + 1, 
									endState.playedCardCount);
			// Test 6: Ensure function returns 0 for successful completion of the cardEffect
			test6Fails += assertTrue("cardEffect returns 0", 0, result);
		...
		totTest6Fails += test6Fails;	// function returning 0
	}
